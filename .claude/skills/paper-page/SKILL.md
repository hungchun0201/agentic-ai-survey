...
content/papers/<slug>.json     ← paper metadata (CI source of truth)
```

Folder name: kebab-case (e.g., `cold-rl`, `deepseek-mla`).

### 2.1 Paper Metadata JSON (CRITICAL)

**Every new paper MUST have a `content/papers/<slug>.json` file.** The CI deploy workflow rebuilds `data/papers-index.json` from `content/papers/*.json` — if you only edit `data/papers-index.json` or `data/papers.json` directly, CI will overwrite your changes on the next push.
...
```json
{
  ...
  "has_repo": true,
  "best_paper": null,
  "summary_zh": "中文摘要（可含 <strong> HTML 標籤）",
  "summary_en": "English summary (may contain <strong> HTML tags)",
...
| Field | Values |
|-------|--------|
| `section` | `s1`–`s7` (see `data/sections.json`) |
| `agentic` | `"yes"`, `"partial"`, `"no"` |
| `best_paper` | `null`, `"best"`, `"hm"`, `"nominee"` |
| `has_detail_page` | `true` if `papers/<slug>/index.html` exists |

**NEVER** edit `data/papers-index.json` directly — it is auto-generated by CI.
...
- [ ] `content/papers/<slug>.json` exists (CI source of truth for site index)
- [ ] **NEVER** edited `data/papers-index.json` directly (CI auto-generates it)
...
1. **For new papers:** ensure `content/papers/<slug>.json` exists (see §2.1). Without it, the paper won't appear in the site index after deploy.
2. `git add papers/<slug>/index.html content/papers/<slug>.json` (and any other changed files)
