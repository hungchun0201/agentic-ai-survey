# CLAUDE.md — vLLM experiments (Remote Lab)
...
**NEVER NEVER NEVER use `kill -9` on vLLM or any GPU process.** It corrupts the CUDA driver state, making the GPU unusable until full machine reboot. There is no recovery short of rebooting.
